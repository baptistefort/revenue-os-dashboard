...
export async function POST(request: Request) {
  const payload = await request.json().catch(() => ({})) as { title?: string; topic?: string };
  const topic = printable(payload.topic?.trim() || "Rapport de direction 2026");
  const title = printable(payload.title?.trim() || topic);
  const documentId = `RAPPORT-${new Date().toISOString().slice(0, 10).replaceAll("-", "")}`;
  ...
  cover.drawText(printable(title), { x: 46, y: 486, size: 31, font: bold, color: NAVY, maxWidth: 475, lineHeight: 36 });
  cover.drawText("Atelier Beaumarchais", { x: 46, y: 432, size: 15, font: regular, color: INK });
  cover.drawText("Décisions, chiffres et priorités reliés à leurs sources.", { x: 46, y: 405, size: 10, font: italic, color: MUTED });
  ...
  cover.drawText("Généré le 15 juillet 2026 · Données de démonstration", { x: 46, y: 72, size: 8, font: regular, color: MUTED });
  ...
  let y = drawParagraph(summary, "L'entreprise maintient une dynamique commerciale positive, mais doit convertir cette croissance en marge et en trésorerie. Le pipeline atteint 184 K€ et le chiffre d'affaires mensuel progresse de 12 %. L'écart principal reste la marge atelier, à 29 % contre un objectif de 32 %.", { x: 42, y: 696, width: 500, font: regular, size: 10.5, color: MUTED });
  y -= 26;
  ...
  y = drawParagraph(summary, "Le chantier Rivoli concentre 82 % de l'écart de marge. Quatorze heures non facturées et un dépassement de 1 438 € sur le placage chêne expliquent l'essentiel du décrochage. Un avenant de 6,8 K€ permettrait de protéger 1,9 point de marge.", { x: 42, y: y - 24, width: 500, font: regular, size: 10.5, color: INK });
  y -= 22;
  summary.drawText("Arbitrage recommandé", { x: 42, y, size: 14, font: bold, color: NAVY });
  drawParagraph(summary, "Valider l'avenant Rivoli avant midi, lancer les deux relances clients déjà préparées et transférer 200 € de Meta vers Google Search. Ces trois décisions combinent un effet immédiat sur la marge, la trésorerie et la création de pipeline.", { x: 42, y: y - 24, width: 500, font: regular, size: 10.5, color: INK });

  const plan = pdf.addPage([PAGE_WIDTH, PAGE_HEIGHT]);
  drawHeader(plan, regular, bold, "Plan d'action", 3);
  plan.drawText("Plan de direction — 90 jours", { x: 42, y: 730, size: 24, font: bold, color: NAVY });
  const actions = [
  ...
  drawParagraph(plan, "STRAT-2026-Q3 · FIN-SNAPSHOT-20260715 · CRM-SNAPSHOT-20260715 · ALERT-201 · PROJET-241 · TEMPS-086 · ACHAT-109 · GADS-2026-07 · META-2026-07", { x: 42, y: 154, width: 500, font: regular, size: 8, color: MUTED, lineHeight: 12 });
